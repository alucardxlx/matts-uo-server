				...
				case 1:
				{
					...
					if ( killer != null && !killer.Deleted )
					{
						...
						if (Core.SE)
						{
							...
							Timer.DelayCall(TimeSpan.FromMinutes(1), new TimerStateCallback(ReportedListExpiry_Callback), new object[] { from, killer }); //changedbyMatt - reduce timer between reports
						}
